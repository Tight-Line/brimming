...
RSpec.describe CommentPolicy do
  ...
  let(:comment) { create(:comment, user: owner, commentable: question) }

  def policy(user)
    described_class.new(user, comment)
  end
  ...
  describe "comment on answer" do
    ...
    let(:answer_comment) { create(:comment, user: owner, commentable: answer) }

    def answer_policy(user)
      described_class.new(user, answer_comment)
    end
    ...
    describe "for space moderators" do
      ...
      it "allows hard_delete through the question's space" do
        expect(answer_policy(moderator).hard_delete?).to be true
      end
    end
  end
  ...
  describe "Scope" do
    ...
    let!(:deleted_comment) { create(:comment, commentable: question, deleted_at: Time.current) }

    it "returns only non-deleted comments" do
      scope = described_class::Scope.new(user, Comment).resolve
      expect(scope).to include(visible_comment)
      expect(scope).not_to include(deleted_comment)
    end

    it "works for visitors" do
      scope = described_class::Scope.new(nil, Comment).resolve
      expect(scope).to include(visible_comment)
      expect(scope).not_to include(deleted_comment)
    end
  end
